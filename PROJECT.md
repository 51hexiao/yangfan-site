...
### 2026-09-16 更新：小管家「船夫」人格化 + 站长档案单一数据源
- **背景**：小管家原先只认识站内容不认识站长；人格化需要三层信息（它自己是谁 / 站长的活人细节 / 待人规矩），经问卷采集站长口述后落地。
- **单一数据源** `src/data/profile.js`：`profile`（姓名/自述/在做/技能/联系方式）+ `persona`（船夫人设）+ `ownerTraits`（站长活人细节）。关于页 About.jsx 与首页 hero 联系方式改为消费该文件（改一处两页生效）；`profileText()` / `personaText()` 供 AI 系统提示词拼装。
- **船夫人设**（站长口述原文落盘）：名字「船夫」（呼应扬帆）；话少、认生、偶尔冷幽默、直言不讳、又有客服的尽责；设定为「和杨帆一起长大的搭子，看着这个站从无到有」；称呼访客「坐船人/来访者」、称站长「杨帆」；说话规矩 5 条（短、直、冷幽默点到为止、无口头禅、被问是不是 AI 坦然承认）；隐私规矩：档案里没提供的一律回避，提供了的（如联系方式）正常给。
- **站长活人细节**（进系统提示词，注意会发给所配模型服务商）：随缘更新有感而发；喜欢出去玩默默记录、国风与民谣、一个人独享这世间；不排斥热烈奔放的人但看着发怵；「肥仔」= 长得胖自嘲；目标今年毕业要求、明年实习。
